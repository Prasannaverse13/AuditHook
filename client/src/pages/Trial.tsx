...
import { sampleSmartContract } from "@/lib/baseUtils";
...
export default function Trial() {
  ...
  const exportReport = () => {
    ...
    const opt = {
      margin: [10, 10, 10, 10],
      filename: `audithook-trial-report-${new Date().toISOString().split('T')[0]}.pdf`,
  ...
  const shareOnTwitter = () => {
    ...
    const text = `I just audited my smart contract with AuditHook AI and got a security score of ${result.securityScore.toFixed(1)}/10 with ${result.issuesCount} issues detected. Try the free trial! #SmartContractSecurity #UniswapV4 #Base`;
