  ...
  return (
    <Card>
        ...
        <div className="space-y-4">
          {contracts.map((contract, index) => (
            <div key={index} className="border rounded-lg p-4">
              ...
              {contract.contractLinks?.viewOnBlockExplorer && (
                <div className="mt-4">
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="text-xs"
                    onClick={() => window.open(contract.contractLinks!.viewOnBlockExplorer, '_blank')}
                  >
      ...
      <CardFooter className="border-t pt-4 text-xs text-gray-500">
        <p>
          These smart contracts were deployed for the AuditHook platform. 
          The contracts are used for demonstrating integration with Base and Uniswap v4 hooks.
        </p>
      </CardFooter>
    </Card>
  );
